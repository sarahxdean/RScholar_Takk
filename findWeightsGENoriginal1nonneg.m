...
function [weight,allr,allrt,allrl,allrh,allrt2,allrl2,allrh2] = findWeightsGENoriginal1nonneg(data_standard,data_light,data_heavy,array,ds_2,dl_2,dh_2,both)
...
for i=1:20    
    ...
    product = productToRegress(tr(:,1:n,1),tr(:,1:n,2));
    ...
    %weight
    rt = testResiduals(ts(:,1:n,1),ts(:,1:n,2),weight,ts(:,n+1,1));
    rl = testResiduals(data_light(:,1:n,1),data_light(:,1:n,2),weight,data_light(:,n+1,1));
    rh = testResiduals(data_heavy(:,1:n,1),data_heavy(:,1:n,2),weight,data_heavy(:,n+1,1));

    if nargin > 4
        if both
            r_t2 = testResiduals(ts_2(:,1:n,1),ts_2(:,1:n,2),weight,ts_2(:,n+1,1),1); %MISRAKE
        else
            r_t2 = testResiduals(ds_2(:,1:n,1),ds_2(:,1:n,2),weight,ds_2(:,n+1,1),1);
        end
        r_l2 = testResiduals(dl_2(:,1:n,1),dl_2(:,1:n,2),weight,dl_2(:,n+1,1),1);
        r_h2 = testResiduals(dh_2(:,1:n,1),dh_2(:,1:n,2),weight,dh_2(:,n+1,1),1);
    end
    ...
    heavy = sqrt(mean(allrh(2:end).^2))c

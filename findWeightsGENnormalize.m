...
for i=1:20      
   ...
   if nargin > 4
    ...
    product = productToRegress(tr(:,1:n,1),tr(:,1:n,2),1);    
        
    [weight,bint,r,rint] = regress(tr(:,n+1,1),product);
    %see residuals
    %see test residuals individually for: remaining 20%, no weight, high
    %weight
    r_t = testResiduals(ts(:,1:n,1),ts(:,1:n,2),weight,ts(:,n+1,1),1);
    r_l = testResiduals(data_light(:,1:n,1),data_light(:,1:n,2),weight,data_light(:,n+1,1),1);
    r_h = testResiduals(data_heavy(:,1:n,1),data_heavy(:,1:n,2),weight,data_heavy(:,n+1,1),1);
    
    if nargin > 4 %SHOULD BE DOING Y-POSITIONS HERE
        if both
    ...
    allr = vertcat(allr,r);
    allrt = vertcat(allrt,r_t);
    allrl = vertcat(allrl,r_l);
    allrh = vertcat(allrh,r_h);
